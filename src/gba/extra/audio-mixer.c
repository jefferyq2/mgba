...
void _mp2kStep(struct GBAAudioMixer* mixer) {
	...
	for (i = 0; i < OVERSAMPLE; ++i) {
		...
		if (mixer->p->externalMixing) {
			blip_add_delta_(mixer->p->psg.left, mixer->p->clock + i * interval, sample.left - mixer->last.left);
			blip_add_delta_(mixer->p->psg.right, mixer->p->clock + i * interval, sample.right - mixer->last.right);
		}
		mixer->last = sample;
	}
}
